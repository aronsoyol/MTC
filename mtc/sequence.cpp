...
sequence::sequence ()
{
	...
	SYSTEMTIME st;
	GetLocalTime(&st);
	sprintf(seg_debugfile, "seqlog-%04d%02d%02d-%02d%02d%0d.txt", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);
#endif
}
...
//
bool sequence::open(char16_t *filename, bool readonly)
{
	return false;
}
...
sequence::buffer_control* sequence::alloc_buffer (size_t maxsize)
{
	...
	// allocate a new buffer of unsigned char/wchar/long/whatever
...
//
sequence::span* sequence::spanfromindex (size_w index, size_w *spanindex = 0)
{
	span* ret =  spanfromindex_fordebug(index, spanindex);
	if(*spanindex == 0 && ret->prev != head)
	{
		STOP;
	}
	assert(*spanindex == 0 ? ret->prev == head : true);
	return ret;
}
sequence::span* sequence::spanfromindex_fordebug (size_w index, size_w *spanindex = 0) 
{
	if(!m_lastUsedSpan || m_lastUsedSpanOffset > index || m_lastUsedSpanOffset + m_lastUsedSpan->length < index )
	{
		span * sptr;
		size_w curidx = 0;
		...
		for(sptr = head->next; sptr->next; sptr = sptr->next)
		{
			if(index >= curidx && index < curidx + sptr->length)
			{
				if(spanindex) 
					*spanindex = curidx;
				m_lastUsedSpan = sptr;
				m_lastUsedSpanOffset = curidx;
				return m_lastUsedSpan;
			}
	...
	{
		if(m_lastUsedSpanOffset == 0)
		{
			int stop =1;
		}
		if(m_lastUsedSpanOffset >= index || m_lastUsedSpanOffset + m_lastUsedSpan->length > index )
		{
			mydebug("1\n");
			*spanindex = m_lastUsedSpanOffset;
			return m_lastUsedSpan;
		}
		else if(m_lastUsedSpanOffset + m_lastUsedSpan->length == index)
		{
			mydebug("1\n");
			m_lastUsedSpanOffset	= m_lastUsedSpanOffset + m_lastUsedSpan->length;
			m_lastUsedSpan			= m_lastUsedSpan->next;
			*spanindex = m_lastUsedSpanOffset;
			return m_lastUsedSpan;
		}
...
void sequence::restore_spanrange (span_range *range, bool undo_or_redo)
{
	...
	if(	range->act == action_erase && undo_or_redo == true || 
		...
		range->act == action_replace && undo_or_redo == true )
	{
...
{
	return (lastaction != action_replace && lastaction == act && lastaction_index == index);
}
...
bool sequence::erase_worker (size_w index, size_w length, action act)
{
	...
	remoffset = index - spanindex;//span中不被删除部分的长度
	removelen = length;
	...
	if(remoffset != 0)
	{
		...
		//std::min
		removelen -= std::min<size_w>(removelen, (sptr->length - remoffset));
	...
	while(removelen > 0 && sptr != tail)
	{
		...
		removelen -= std::min<size_w>(removelen, sptr->length);
...
bool sequence::replace_group_no_change(size_w index, const seqchar *buf, size_w length, size_w erase_length)
{
	size_t remlen = 0;
	...
	remlen = std::min<size_w>(sequence_length - index, erase_length);
...
//
//	Returns number of seqchars copied into destination
//
size_w sequence::render(size_w index, seqchar *dest, size_w length) /*const*/
{
	...
	size_w spanoffset2 = index - spanoffset;
	//mydebug("index:%10d spanoffset: %10d ,index - spanoffset = %10d\n", index, spanoffset,spanoffset2);
	spanoffset = spanoffset2;
	while(length && sptr != tail)
	{
		size_w copylen   = std::min<size_w>(sptr->length - spanoffset, length);
...
//
seqchar sequence::peek(size_w index) 
{
	seqchar   value;
...
//
//seqchar sequence::operator[] (size_w index) 
//{
//	return peek(index);
//}
//
