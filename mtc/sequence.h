...
#define MTC_DOC_SEQ_START namespace MTC{namespace DOC{namespace SEQ{
#define MTC_DOC_SEQ_END }}}

MTC_DOC_SEQ_START
//
//	Define the underlying string/character type of the sequence.
//
//	'seqchar' can be redefined to unsigned char, char16_t, uint32_t etc 
//	depending on what kind of string you want your sequence to hold
//
typedef char16_t		seqchar;
...
const size_w MAX_SEQUENCE_LENGTH = (std::numeric_limits<size_w>::max());
...
public:
	...
	//
	bool		init();
	bool		open(char16_t *filename, bool readonly);
	bool		clear();
	...
	bool		replace_group_no_change(size_w index, const seqchar *buf, size_w length, size_w erase_length);
...
private:
	...
	span		*	spanfromindex(size_w index, size_w *spanindex) ;
	span		*	spanfromindex_fordebug(size_w index, size_w *spanindex) ;
	void			scan(span *sptr);
	size_w			sequence_length;
	span		*	head;
	span		*	tail;	
